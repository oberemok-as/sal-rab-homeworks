...
function sendRequest(name, phone, address, goods, sum) {
    let datamain = {data: { client: '', goods: [], order: {} }};
    
    let countOfGoods = goods.length;

    for (let i = 0; i < countOfGoods; i += 1) {
        datamain.data.goods.push({ title: goods[i].title, count: goods[i].count });

    }

    datamain.data.order['address'] = `ул. ${address.street}, дом ${address.house}, ${address.entrance} подъезд, ${address.floor} этаж, кв. ${address.flat}`;
    datamain.data.order.sum = sum;

    datamain.data['client'] = name + ' ' + phone;

    let jsonData = JSON.stringify(datamain);
